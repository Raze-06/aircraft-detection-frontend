...
interface Blip {
  ...
  x: number;
  y: number;
  type: "aircraft" | "helicopter" | "fighter";
}
...
const RadarDisplay = ({ isScanning = true, blips: externalBlips, size = 300 }: RadarDisplayProps) => {
  const [blips, setBlips] = useState<Blip[]>(externalBlips || []);
    ...
    const interval = setInterval(() => {
      setBlips((prev) => {
        ...
        newBlips.push({
          ...
          type: ["aircraft", "helicopter", "fighter"][Math.floor(Math.random() * 3)] as Blip["type"],
        });
        return newBlips;
      });
    }, 2000);
